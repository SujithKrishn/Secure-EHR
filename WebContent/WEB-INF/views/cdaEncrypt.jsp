...
<body>
<h1>
	Encrypt Patient data and Data Transfer
</h1>

<c:url var="addAction" value="/cda/encryptData" ></c:url>
...
<%-- 			<form:input path="dob" /> --%>
	...
	<tr>
		<td colspan="2">
			<c:if test="${!empty person.name}">
				<input type="submit"
					value="<spring:message text="Encrypt Person data and Transfer" />" />
			</c:if>
...
<c:if test="${!empty listPersons}">
	...
	<c:forEach items="${listPersons}" var="person">
		<tr>
		<td><a href="<c:url value='/editCda/${person.id}' />" >Edit</a></td>
			<td>${person.id}</td>
			<td>${person.name}</td>
